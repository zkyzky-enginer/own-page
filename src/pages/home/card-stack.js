  ...
  return (
    <div className="card-stack-container" ref={containerRef}>
			{cards.map((card, index) => (
				...
				>
					<div className="stack-desc flex flex-col w-[25%] h-full leading-[22px] justify-center p-[2rem] bg-[#FF8C0046] ">
					...
					<div className="flex-1 w-full h-full relative bg-[#FF8C0016] p-[2rem]">
						{/* 引入porject-img下的图片 */}
						<img src={`./img/project-img/${card.img}`} className="w-full h-[85%] border-1 border-[#e8e8e8] rounded-lg" alt="项目图片1" />
						<img src={`./img/project-img/${card.otherImg}`}  className="absolute bottom-20px -right-[40px] w-[420px] border-1 border-[#e8e8e8] rounded-lg" alt="项目图片2" />
					</div>
				</div>
			))}
    </div>
